...
public class Jogo2 extends javax.swing.JFrame {
 ...
 // chamar o metodo adicionar     
  ...
  // verificar e´se o jogo foi finalizado
  if(jogoEncerrado) return;
  
 //verificar se o botao ja foi aberto
 if(abertos[linha][coluna]) return;
 
 /*se o jogo estiver rodando e o butão ainda não tiver
 sido aberto - então vomos abrir o botão */
 abertos[linha][coluna]=true;
 quantidadeCasasAbertas++;
 
 // acessar o que tem dentro do botão
 JButton botao = btnCampos[linha][coluna];
 //se no botão uma bomba, então vomos mostra a bomba a ele
 if(bombas[linha][coluna]){
    ImageIcon imgBomba = new ImageIcon( getClass().getResource("/Interface/bonb(1).png"));
    //colocar aimagem no botao
    botao.setIcon(imgBomba);
    return;
    
 }else{
      ImageIcon imgBandeira = new ImageIcon( getClass().getResource("/Interface/flag(1).png"));
